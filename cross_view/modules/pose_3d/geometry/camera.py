    ...
    def load_from_file(fname, cid):
        ...
        if "P" in data:
            ...
            if 'H' in data:
                ...
                H = np.array(data['Hi2w'])
            
            w = data['w']
            h = data['h']
            cam = AffineCamera(P, w, h, cid)
            cam.H = H 
            return cam
        elif "K" in data:
            K = np.array(data['K'])
            ...
            w = data['w']
            h = data['h']
            cam = ProjectiveCamera(K, rvec, tvec, distCoef, w, h, cid)
            return cam
        else:
...
class AffineCamera(Camera):
    """
        Affine camera
    """
    def __init__(self, P, w, h, cid):
        Camera.__init__(self, P, w, h, cid=cid)

class ProjectiveCamera(Camera):
    """
        Projective camera
    """
    def __init__(self, K, rvec, tvec, distCoef, w, h, cid):
